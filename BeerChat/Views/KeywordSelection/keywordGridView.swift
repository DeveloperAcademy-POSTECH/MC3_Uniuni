    ...
    var body: some View {
        LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 10), count: 3), spacing: 10) {
            ForEach(keyword, id: \.self) { word in
                VStack {
                    Button(action: {
                        if !seletedKewords.contains(word) && seletedKewords.count < 5 {
                            seletedKewords.insert(word)
                        } else {
...
struct KeywordGridView_Previews: PreviewProvider {
    static var previews: some View {
        KeywordGridView(keyword: ["학점 4.0", "학점 3.0", "전과", "공대", "미대", "외국어대", "전자정보대"], seletedKewords: .constant([]))
    }
}
